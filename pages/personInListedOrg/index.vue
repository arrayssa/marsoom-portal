<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('personInListedOrg') }}</h2>
  ...
  <addEditPersonInListedOrg v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditPersonInListedOrg = defineAsyncComponent(() => import('./add-edit-personInListedOrg.vue'));
...
const { pending, data, refresh } = useGetApi(`organization-persons`, {
  limit: perPage,
  page: currentPage
});
...
const onRowClick = (event) => {
  navigateTo(localePath(`/personInListedOrg/details/${event.data.id}`));
};
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`organization-persons/${selectedId.value}`);
        refresh();
