<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('shipper') }}</h2>
    <LocLink :to="'/shipper/add-edit'">
...
const { pending, data, refresh } = useGetApi(`shipper`, {
  limit: perPage,
  page: currentPage
});
...
const actionsMenu = computed(() => [
  {
    ...
    icon: 'pi pi-pencil',
    command: () => {
      navigateTo(localePath(`/shipper/add-edit/${selectedId.value}`));
    }
  },
  {
    type: 'delete',
...
const onRowClick = (event) => {
  navigateTo(localePath(`/shipper/details/${event.data.id}`));
};
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`shipper/${selectedId.value}`);
        refresh();
