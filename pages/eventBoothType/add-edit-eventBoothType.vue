<template>
  <Dialog
    ...
    modal
    :header="id ? $t('editForm', { name: $t('eventBoothType') }) : $t('addNewForm.feminine', { name: $t('eventBoothType') })"
    :style="{ width: '50rem' }"
      ...
      <div class="grid">
        <div class="col-12">
          <div class="flex flex-column gap-2">
            <AppInput v-model="eventBoothTypeForm.name" :name="'name'" :label="$t('name')" :errors="errors" :rules="'required'" />
          </div>
        </div>
        <div class="col-6">
...
let eventBoothTypeForm = ref({
  name: ''
});

const { pending, execute } = useGetApi(
  `event_booth_type/${props.id}`,
  {},
  {
    immediate: false,
    transform: (res) => {
      eventBoothTypeForm.value = res.data;
    }
  }
);
...
const onSubmit = async (value) => {
  ...
  try {
    if (props.id) {
      await usePutApi(`event_booth_type/${props.id}`, value);
      closeModal();
      emit('submit');
    } else {
      await usePostApi('event_booth_type', value);
      closeModal();
      emit('submit');
    }
