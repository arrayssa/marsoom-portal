<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('eventBoothType') }}</h2>
  ...
  <addEditEventBoothType v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditEventBoothType = defineAsyncComponent(() => import('./add-edit-eventBoothType.vue'));
...
const { pending, data, refresh } = useGetApi(`event_booth_type`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`event_booth_type/${selectedId.value}`);
        refresh();
