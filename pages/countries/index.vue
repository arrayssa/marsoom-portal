<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('countries') }}</h2>
  ...
  <DataTable :loading="pending" :value="data?.data?.rows" lazy paginator :totalRecords="data?.data?.meta.total" :rows="perPage" @page="handlePageChange" tableStyle="min-width: 50rem">
    ...
    <Column field="initials" :header="$t('initials')"></Column>
    <Column field="flag" :header="$t('flag')">
    ...
    <Column field="nationality" :header="$t('nationality')"></Column>
  ...
  <addEditCountry v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditCountry = defineAsyncComponent(() => import('./add-edit-country.vue'));
...
const { pending, data, refresh } = useGetApi(`countries`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`countries/${selectedId.value}`);
        refresh();
