<template>
  <Dialog
    ...
    modal
    :header="id ? $t('editForm', { name: $t('eventBooth') }) : $t('addNewForm.feminine', { name: $t('eventBooth') })"
    :style="{ width: '50rem' }"
      ...
      <div class="grid">
        <div class="col-12">
          <div class="flex flex-column gap-2">
            <AppInput v-model="eventBoothForm.name" :name="'name'" :label="$t('name')" :errors="errors" :rules="'required'" />
          </div>
        </div>
        <div class="col-6">
...
let eventBoothForm = ref({
  name: ''
});

const { pending, execute } = useGetApi(
  `event-booth/${props.id}`,
  {},
  {
    immediate: false,
    transform: (res) => {
      eventBoothForm.value = res.data;
    }
  }
);
...
const onSubmit = async (value) => {
  ...
  try {
    if (props.id) {
      await usePutApi(`event-booth/${props.id}`, value);
      closeModal();
      emit('submit');
    } else {
      await usePostApi('event-booth', value);
      closeModal();
      emit('submit');
    }
