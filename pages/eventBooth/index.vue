<template>
    <div class="flex align-items justify-content-between mt-5 mb-3">
      <h2 class="text-xl">{{ $t('eventBooth') }}</h2>
    ...
    <addEditEventBooth v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
    <ConfirmDialog></ConfirmDialog>
  </template>
  
  <script setup>
  const addEditEventBooth = defineAsyncComponent(() => import('./add-edit-eventBooth.vue'));
  ...
  const { pending, data, refresh } = useGetApi(`event-booth`, {
    limit: perPage,
    page: currentPage
  });
  ...
  const deleteConfirm = () => {
    confirm.require({
      ...
      accept: async () => {
        try {
          await useDeleteApi(`event-booth/${selectedId.value}`);
          refresh();
