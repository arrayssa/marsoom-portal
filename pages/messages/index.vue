<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('messages') }}</h2>
  ...
  <addEditMessage v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditMessage = defineAsyncComponent(() => import('./add-edit-message.vue'));
...
const { pending, data, refresh } = useGetApi(`messages`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`messages/${selectedId.value}`);
        refresh();
