<template>
  <Dialog
    ...
    modal
    :header="id ? $t('editForm', { name: $t('eventSpace') }) : $t('addNewForm.feminine', { name: $t('eventSpace') })"
    :style="{ width: '50rem' }"
      ...
      <div class="grid">
        <div class="col-12">
          <div class="flex flex-column gap-2">
            <AppInput v-model="eventSpaceForm.name" :name="'name'" :label="$t('name')" :errors="errors" :rules="'required'" />
          </div>
        </div>
        <div class="col-6">
...
let eventSpaceForm = ref({
  name: ''
});

const { pending, execute } = useGetApi(
  `event-space/${props.id}`,
  {},
  {
    immediate: false,
    transform: (res) => {
      eventSpaceForm.value = res.data;
    }
  }
);
...
const onSubmit = async (value) => {
  ...
  try {
    if (props.id) {
      await usePutApi(`event-space/${props.id}`, value);
      closeModal();
      emit('submit');
    } else {
      await usePostApi('event-space', value);
      closeModal();
      emit('submit');
    }
