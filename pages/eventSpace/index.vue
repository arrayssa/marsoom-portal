<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('eventSpace') }}</h2>
  ...
  <addEditEventSpace v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditEventSpace = defineAsyncComponent(() => import('./add-edit-eventSpace.vue'));
...
const { pending, data, refresh } = useGetApi(`event-space`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`event-space/${selectedId.value}`);
        refresh();
