<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('messagesClassfication') }}</h2>
  ...
  <addEditMessagesClassfication v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditMessagesClassfication = defineAsyncComponent(() => import('./add-edit-messagesClassfication.vue'));
...
const { pending, data, refresh } = useGetApi(`messages-classfication`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`messages-classfication/${selectedId.value}`);
        refresh();
