<template>
  <div class="flex align-items-center justify-content-start mt-5 mb-5">
    ...
    <h2 class="text-xl mb-0 mt-0">{{ eventId ? $t('editForm', { name: $t('accompanyingEvents') }) : $t('addNewForm.feminine', { name: $t('accompanyingEvents') }) }}</h2>
  </div>
  ...
  <Form @submit="onSubmit" v-slot:default="{ errors }" v-else>
    ...
    <div class="flex justify-content-end mb-4 mt-4 gap-3">
      <Button severity="secondary" @click="navigateTo(localePath(`/accompanyingEvents`))" size="large">{{ $t('cancel') }}</Button>
...
const onSubmit = async (value) => {
  ...
  try {
    if (eventId) {
      await usePutApi(`accompanying-event/${eventId}`, value);
      navigateTo(localePath(`/accompanyingEvents`));
    } else {
      await usePostApi('accompanying-event', value);
      navigateTo(localePath(`/accompanyingEvents`));
    }
