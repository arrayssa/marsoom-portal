<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('manifest') }}</h2>
  ...
  <addEditManifest v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditManifest = defineAsyncComponent(() => import('./add-edit-manifest.vue'));
...
const { pending, data, refresh } = useGetApi(`manifest`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`manifest/${selectedId.value}`);
        refresh();
