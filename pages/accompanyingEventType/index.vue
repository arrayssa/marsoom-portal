<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('accompanyingEventType') }}</h2>
  ...
  <DataTable :loading="pending" :value="data?.data?.rows" lazy paginator :totalRecords="data?.data?.meta.total" :rows="perPage" @page="handlePageChange" tableStyle="min-width: 50rem">
    ...
    <Column field="specifications" :header="$t('specifications')"></Column>
  ...
  <addEditAccompanyingEventType v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditAccompanyingEventType = defineAsyncComponent(() => import('./add-edit-accompanyingEventType.vue'));
...
const { pending, data, refresh } = useGetApi(`accompanying-event-list-types`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`accompanying-event-list-types/${selectedId.value}`);
        refresh();
