<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('eventSeries') }}</h2>
  ...
  <addEditEventSeries v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditEventSeries = defineAsyncComponent(() => import('./add-edit-eventSeries.vue'));
...
const { pending, data, refresh } = useGetApi(`event-series`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`event-series/${selectedId.value}`);
        refresh();
