<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('events') }}</h2>
    <LocLink :to="'/events/add-edit'">
  ...
  <DataTable :loading="pending" :value="data?.data?.rows" lazy paginator :totalRecords="data?.data?.meta.total" :rows="perPage" @page="handlePageChange" @row-click="onRowClick" tableStyle="min-width: 50rem">
    ...
    <Column field="street_adress" :header="$t('address')"></Column>
    <Column field="start_date" :header="$t('date')"></Column>
...
const { pending, data, refresh } = useGetApi(`events`, {
  limit: perPage,
  page: currentPage
});
...
const actionsMenu = computed(() => [
  {
    ...
    icon: 'pi pi-pencil',
    command: () => {
      navigateTo(localePath(`/events/add-edit/${selectedId.value}`));
    }
  },
  {
    type: 'delete',
...
const onRowClick = (event) => {
  navigateTo(localePath(`/events/details/${event.data.id}`));
};
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`events/${selectedId.value}`);
        refresh();
