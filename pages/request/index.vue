<template>
  ...
  <addEditRequest v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditRequest = defineAsyncComponent(() => import('./add-edit-request.vue'));
...
const { pending, data, refresh } = useGetApi(`request`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`request/${selectedId.value}`);
        refresh();
