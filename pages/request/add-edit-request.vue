<template>
  <Dialog
    ...
    modal
    :header="id ? $t('editForm', { name: $t('request') }) : $t('addNewForm.feminine', { name: $t('request') })"
    :style="{ width: '50rem' }"
      ...
      <div class="grid">
        <div class="col-12">
          <div class="flex flex-column gap-2">
            <AppInput v-model="requestForm.name" :name="'name'" :label="$t('name')" :errors="errors" :rules="'required'" />
          </div>
        </div>
        <div class="col-6">
...
const onSubmit = async (value) => {
  ...
  try {
    if (props.id) {
      await usePutApi(`request/${props.id}`, value);
      closeModal();
      emit('submit');
    } else {
      await usePostApi('request', value);
      closeModal();
      emit('submit');
    }
