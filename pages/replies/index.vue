<template>
  ...
  <addEditReply v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditReply = defineAsyncComponent(() => import('./add-edit-reply.vue'));
...
const { pending, data, refresh } = useGetApi(`replies`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`replies/${selectedId.value}`);
        refresh();
