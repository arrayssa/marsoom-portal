<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('itemInPackage') }}</h2>
  ...
  <addEditItemInPackage v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditItemInPackage = defineAsyncComponent(() => import('./add-edit-itemInPackage.vue'));
...
const { pending, data, refresh } = useGetApi(`item-in-package`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`item-in-package/${selectedId.value}`);
        refresh();
