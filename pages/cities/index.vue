<template>
  ...
  <addEditCity v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditCity = defineAsyncComponent(() => import('./add-edit-city.vue'));
...
const { pending, data, refresh } = useGetApi(`cities`, {
  limit: perPage,
  page: currentPage
});
...
const toggle = (event, id, actionType) => {
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`cities/${selectedId.value}`);
        refresh();
