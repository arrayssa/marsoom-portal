<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('interest') }}</h2>
...
<script setup>
const addEditInterest = defineAsyncComponent(() => import('./add-edit-interest.vue'));
...
const { pending, data, refresh } = useGetApi(`interests`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`interests/${selectedId.value}`);
        refresh();
