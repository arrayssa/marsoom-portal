<template>
  <Dialog
    ...
    modal
    :header="id ? $t('editForm', { name: $t('interest') }) : $t('addNewForm.feminine', { name: $t('interest') })"
    :style="{ width: '50rem' }"
      ...
      <div class="grid">
        <div class="col-12">
          <div class="flex flex-column gap-2">
            <AppInput v-model="interestForm.name" :name="'name'" :label="$t('name')" :errors="errors" :rules="'required'" />
          </div>
        </div>
        <div class="col-6">
...
const { pending, execute } = useGetApi(
  ...
  {},
  {
    immediate: false,
    transform: (res) => {
      interestForm.value = res.data;
      interestForm.value.country_id = Number(res.data?.country?.id);
    }
  }
);
...
const onSubmit = async (value) => {
  ...
  try {
    if (props.id) {
      await usePutApi(`interests/${props.id}`, value);
      closeModal();
      emit('submit');
    } else {
      await usePostApi('interests', value);
      closeModal();
      emit('submit');
    }
