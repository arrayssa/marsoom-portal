<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('itemInEvents') }}</h2>
  ...
  <DataTable :loading="pending" :value="data?.data?.rows" lazy paginator :totalRecords="data?.data?.meta.total" :rows="perPage" @page="handlePageChange" @row-click="onRowClick" tableStyle="min-width: 50rem">
    ...
    <Column field="title" :header="$t('autherName')"></Column>
    <Column field="content_summary" :header="$t('contentSummary')"></Column>
    <Column field="price" :header="$t('price')"></Column>
    <Column field="organization_discount" :header="$t('discount')"></Column>
    <Column field="number_of_copies" :header="$t('numberOfCopies')"></Column>
    ...
    <Column field="review" :header="$t('review')">
      <template #body="slotProps">
        <Badge v-if="slotProps.data.review && slotProps.data.review != '0'" value="Yes"></Badge>
        <Badge v-else value="No" severity="danger"></Badge>
      </template>
    </Column>
  ...
  <addEditItemInEvents v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditItemInEvents = defineAsyncComponent(() => import('./add-edit-itemInEvents.vue'));
...
const { pending, data, refresh } = useGetApi(`event-items`, {
  limit: perPage,
  page: currentPage
});
...
const onRowClick = (event) => {
  navigateTo(localePath(`/itemInEvents/details/${event.data.id}`));
};
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`event-items/${selectedId.value}`);
        refresh();
