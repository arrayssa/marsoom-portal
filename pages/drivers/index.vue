<template>
  ...
  <addEditDriver v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditDriver = defineAsyncComponent(() => import('./add-edit-driver.vue'));
...
const { pending, data, refresh } = useGetApi(`drivers`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`drivers/${selectedId.value}`);
        refresh();
