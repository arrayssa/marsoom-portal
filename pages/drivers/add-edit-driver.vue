<template>
  <Dialog
    ...
    modal
    :header="id ? $t('editForm', { name: $t('driver') }) : $t('addNewForm.feminine', { name: $t('driver') })"
    :style="{ width: '50rem' }"
      ...
      <div class="grid">
        <div class="col-12">
          <div class="flex flex-column gap-2">
            <AppInput v-model="driversForm.name" :name="'name'" :label="$t('name')" :errors="errors" :rules="'required'" />
          </div>
        </div>
        <div class="col-6">
...
const onSubmit = async (value) => {
  ...
  try {
    if (props.id) {
      await usePutApi(`drivers/${props.id}`, value);
      closeModal();
      emit('submit');
    } else {
      await usePostApi('drivers', value);
      closeModal();
      emit('submit');
    }
