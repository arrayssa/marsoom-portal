<template>
  ...
  <!-- <addEditPackage v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" /> -->
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
// const addEditPackage = defineAsyncComponent(() => import('./add-edit-package.vue'));
...
const { pending, data, refresh } = useGetApi(`package`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`package/${selectedId.value}`);
        refresh();
