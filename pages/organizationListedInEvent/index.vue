<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('organizationListedInEvent') }}</h2>
  ...
  <DataTable :loading="pending" :value="data?.data?.rows" lazy paginator :totalRecords="data?.data?.meta.total" :rows="perPage" @page="handlePageChange" @row-click="onRowClick" tableStyle="min-width: 50rem">
    ...
    <Column field="raranking" :header="$t('raranking')"></Column>
  ...
  <addEditOrganizationListedInEvent v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditOrganizationListedInEvent = defineAsyncComponent(() => import('./add-edit-organizationListedInEvent.vue'));
...
const { pending, data, refresh } = useGetApi(`event-organizations`, {
  limit: perPage,
  page: currentPage
});
...
const onRowClick = (event) => {
  navigateTo(localePath(`/organizationListedInEvent/details/${event.data.id}`));
};
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`event-organizations/${selectedId.value}`);
        refresh();
