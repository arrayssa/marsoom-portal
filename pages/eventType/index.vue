<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('eventType') }}</h2>
  ...
  <addEditEventType v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditEventType = defineAsyncComponent(() => import('./add-edit-eventType.vue'));
...
const { pending, data, refresh } = useGetApi(`event-types`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`event-types/${selectedId.value}`);
        refresh();
