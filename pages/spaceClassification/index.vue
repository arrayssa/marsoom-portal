<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('spaceClassification') }}</h2>
  ...
  <addEditSpaceClassification v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditSpaceClassification = defineAsyncComponent(() => import('./add-edit-spaceClassification.vue'));
...
const { pending, data, refresh } = useGetApi(`space-classification`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`space-classification/${selectedId.value}`);
        refresh();
