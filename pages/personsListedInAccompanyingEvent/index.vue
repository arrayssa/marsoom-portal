<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('personsListedInAccompanyingEvent') }}</h2>
  ...
  <addEditPersonsListedInAccompanyingEvent v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditPersonsListedInAccompanyingEvent = defineAsyncComponent(() => import('./add-edit-personsListedInAccompanyingEvent.vue'));
...
const { pending, data, refresh } = useGetApi(`person-in-accompanying-events`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`person-in-accompanying-events/${selectedId.value}`);
        refresh();
