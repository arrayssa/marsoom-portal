<template>
  <Dialog
    ...
    modal
    :header="id ? $t('editForm', { name: $t('personsListedInAccompanyingEvent') }) : $t('addNewForm.feminine', { name: $t('personsListedInAccompanyingEvent') })"
    :style="{ width: '50rem' }"
      ...
      <div class="grid">
        <div class="col-12">
          <div class="flex flex-column gap-2">
            <AppInput v-model="personForm.name" :name="'name'" :label="$t('name')" :errors="errors" :rules="'required'" />
          </div>
        </div>
        <div class="col-6">
...
const { pending, execute } = useGetApi(
  `person-in-accompanying-events/${props.id}`,
  {},
  {
    immediate: false,
    transform: (res) => {
...
const onSubmit = async (value) => {
  ...
  try {
    if (props.id) {
      await usePutApi(`person-in-accompanying-events/${props.id}`, value);
      closeModal();
      emit('submit');
    } else {
      await usePostApi('person-in-accompanying-events', value);
      closeModal();
      emit('submit');
    }
