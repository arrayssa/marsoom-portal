<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('listedType') }}</h2>
  ...
  <DataTable :loading="pending" :value="data?.data?.rows" lazy paginator :totalRecords="data?.data?.meta.total" :rows="perPage" @page="handlePageChange" tableStyle="min-width: 50rem">
    ...
    <Column field="specification" :header="$t('description')"></Column>
  ...
  <addEditListedType v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditListedType = defineAsyncComponent(() => import('./add-edit-listedType.vue'));
...
const { pending, data, refresh } = useGetApi(`list-types`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`list-types/${selectedId.value}`);
        refresh();
