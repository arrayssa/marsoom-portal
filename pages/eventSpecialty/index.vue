<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('eventSpecialty') }}</h2>
  ...
  <addEditEventSpecialty v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditEventSpecialty = defineAsyncComponent(() => import('./add-edit-eventSpecialty.vue'));
...
const { pending, data, refresh } = useGetApi(`event-speciality`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`event-speciality/${selectedId.value}`);
        refresh();
