<template>
    ...
    <addEditStorage v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
    <ConfirmDialog></ConfirmDialog>
  </template>
  
  <script setup>
  const addEditStorage = defineAsyncComponent(() => import('./add-edit-storage.vue'));
  ...
  const { pending, data, refresh } = useGetApi(`storage`, {
    limit: perPage,
    page: currentPage
  });
  ...
  const deleteConfirm = () => {
    confirm.require({
      ...
      accept: async () => {
        try {
          await useDeleteApi(`storage/${selectedId.value}`);
          refresh();
