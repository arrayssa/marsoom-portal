<template>
    <Dialog
      ...
      modal
      :header="id ? $t('editForm', { name: $t('storage') }) : $t('addNewForm.feminine', { name: $t('storage') })"
      :style="{ width: '50rem' }"
        ...
        <div class="grid">
          <div class="col-12">
            <div class="flex flex-column gap-2">
              <AppInput v-model="storageForm.name" :name="'name'" :label="$t('name')" :errors="errors" :rules="'required'" />
            </div>
          </div>
          <div class="col-6">
  ...
  const onSubmit = async (value) => {
    ...
    try {
      if (props.id) {
        await usePutApi(`storage/${props.id}`, value);
        closeModal();
        emit('submit');
      } else {
        await usePostApi('storage', value);
        closeModal();
        emit('submit');
      }
