<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('invoices') }}</h2>
  ...
  <addEditInvoice v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <detailsInvoice v-if="visibleDetails" :id="selectedId" @close-details-modal="closeDetilsModal" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditInvoice = defineAsyncComponent(() => import('./add-edit-invoice.vue'));
const detailsInvoice = defineAsyncComponent(() => import('./details-invoice.vue'));
...
const { pending, data, refresh } = useGetApi(`invoices`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`invoices/${selectedId.value}`);
        refresh();
