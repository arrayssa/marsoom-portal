<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('persons') }}</h2>
    <LocLink :to="'/persons/add-edit'">
  ...
  <DataTable :loading="pending" :value="data?.data?.rows" lazy paginator :totalRecords="data?.data?.meta.total" :rows="perPage" @page="handlePageChange"  @row-click="onRowClick" tableStyle="min-width: 50rem">
    ...
    <Column field="name" :header="$t('name')">
      <template #body="slotProps">
        {{ slotProps.data.first_name + ' ' + slotProps.data.surName }}
      </template>
    </Column>
    <Column field="id_type" :header="$t('idType')"></Column>
...
const { pending, data, refresh } = useGetApi(`persons`, {
  limit: perPage,
  page: currentPage
});
...
const actionsMenu = computed(() => [
  {
    ...
    icon: 'pi pi-pencil',
    command: () => {
      navigateTo(localePath(`/persons/add-edit/${selectedId.value}`));
    }
  },
  {
    type: 'delete',
...
const onRowClick = (event) => {
  navigateTo(localePath(`/persons/details/${event.data.id}`));
}
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`persons/${selectedId.value}`);
        refresh();
