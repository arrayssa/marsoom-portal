<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('accompanyingListedType') }}</h2>
  ...
  <addEditAccompanyingListedType v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditAccompanyingListedType = defineAsyncComponent(() => import('./add-edit-accompanyingListedType.vue'));
...
const { pending, data, refresh } = useGetApi(`accompanying-list-types`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`accompanying-list-types/${selectedId.value}`);
        refresh();
