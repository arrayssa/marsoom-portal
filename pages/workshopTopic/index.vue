<template>
  <div class="flex align-items justify-content-between mt-5 mb-3">
    <h2 class="text-xl">{{ $t('workshopTopic') }}</h2>
  ...
  <addEditWorkshop v-if="visible" :id="selectedId" @close-modal="closeModal" @submit="refresh" />
  <ConfirmDialog></ConfirmDialog>
</template>

<script setup>
const addEditWorkshop = defineAsyncComponent(() => import('./add-edit-workshop.vue'));
...
const { pending, data, refresh } = useGetApi(`work-topics`, {
  limit: perPage,
  page: currentPage
});
...
const deleteConfirm = () => {
  confirm.require({
    ...
    accept: async () => {
      try {
        await useDeleteApi(`work-topics/${selectedId.value}`);
        refresh();
