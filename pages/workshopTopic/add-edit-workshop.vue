<template>
  <Dialog
    ...
    modal
    :header="id ? $t('editForm', { name: $t('workshopTopic') }) : $t('addNewForm.feminine', { name: $t('workshopTopic') })"
    :style="{ width: '50rem' }"
      ...
      <div class="grid">
        <div class="col-12">
          <div class="flex flex-column gap-2">
            <AppInput v-model="workshopForm.name" :name="'name'" :label="$t('name')" :errors="errors" :rules="'required'" />
          </div>
        </div>
        <div class="col-6">
...
let workshopForm = ref({
  name: ''
});

const { pending, execute } = useGetApi(
  `work-topics/${props.id}`,
  {},
  {
    immediate: false,
    transform: (res) => {
      workshopForm.value = res.data;
    }
  }
);
...
const onSubmit = async (value) => {
  ...
  try {
    if (props.id) {
      await usePutApi(`work-topics/${props.id}`, value);
      closeModal();
      emit('submit');
    } else {
      await usePostApi('work-topics', value);
      closeModal();
      emit('submit');
    }
